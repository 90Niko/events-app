...
export default function Page() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({ type: "ok", text: "Event created successfully 🎉" });
        form.reset();
      } else {
          ...
          <form onSubmit={onSubmit} className="p-5 md:p-8">
            ...
            <div className="mt-4">
              <FieldTextarea
                label="Description"
                name="description"
                placeholder="Short description, rules, what to bring, etc."
                hint="Markdown supported in some UIs."
              />
            </div>
            ...
            <div className="mt-6 flex flex-col items-start gap-3 md:flex-row">
              <div className="w-full md:w-64">
                ...
                <select
                  id="status"
                  name="status"
                  className="mt-1 block w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-slate-800 outline-none ring-0 shadow-sm
                             focus-visible:border-slate-300 focus-visible:ring-4 focus-visible:ring-slate-100 transition"
                  defaultValue=""
                >
              ...
              <div className="md:ml-auto">
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-2.5 text-white shadow-sm transition
  ...
  return (
    <div>
      ...
      <input
        ...
        className="mt-1 block w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-slate-800 outline-none shadow-sm
                   placeholder:text-slate-400 ring-0 focus-visible:border-slate-300 focus-visible:ring-4 focus-visible:ring-slate-100 transition"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <textarea
        ...
        placeholder={placeholder}
        rows={4}
        className="mt-1 block w-full resize-y rounded-2xl border border-slate-200 bg-white px-3 py-2 text-slate-800 outline-none shadow-sm
                   placeholder:text-slate-400 ring-0 focus-visible:border-slate-300 focus-visible:ring-4 focus-visible:ring-slate-100 transition"
      />
    </div>
  );
