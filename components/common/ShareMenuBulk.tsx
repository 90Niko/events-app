...
export default function ShareMenuBulk({ kind, params }: { kind: "expense" | "income"; params?: { start?: string; end?: string; category?: string } }) {
  ...
  if (params?.start) qsBase.set("start", params.start);
  if (params?.end) qsBase.set("end", params.end);
  if (params?.category) qsBase.set("category", params.category);
  const qsCsv = new URLSearchParams(qsBase); qsCsv.set("format", "csv");
  const qsXls = new URLSearchParams(qsBase); qsXls.set("format", "excel");
  const qsDoc = new URLSearchParams(qsBase); qsDoc.set("format", "word");
  const qsPrint = new URLSearchParams(qsBase);
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" onClick={() => setOpen(false)}>
        <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mt-3 grid grid-cols-1 gap-2">
            <a className="btn-outline justify-start" href={`${base}?${qsCsv.toString()}`} onClick={() => setOpen(false)}>
            ...
            <a className="btn-outline justify-start" href={`${base}?${qsXls.toString()}`} onClick={() => setOpen(false)}>
            ...
            <a className="btn-outline justify-start" href={`${base}?${qsDoc.toString()}`} onClick={() => setOpen(false)}>
            ...
            <a className="btn-outline justify-start" href={`${printBase}?${qsPrint.toString()}`} target="_blank" rel="noreferrer" onClick={() => setOpen(false)}>
  ...
  return (
    <>
      <button type="button" className="btn-outline inline-flex items-center gap-1" onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open} aria-label="Export / Print">
        <svg aria-hidden viewBox="0 0 24 24" className="h-4 w-4"><path fill="currentColor" d="M15 8a3 3 0 11-2.83 4H9a3 3 0 110-2h3.17A3 3 0 0115 8zm0-5a2 2 0 110 4 2 2 0 010-4zM6 10a2 2 0 110 4 2 2 0 010-4z"/></svg>
